  ...
  return <section className="hero-shell">
    ...
    <div className="mx-auto flex max-w-6xl flex-col gap-10 px-5 pb-20 pt-32 sm:px-8 sm:pb-28 sm:pt-40 lg:grid lg:grid-cols-[1fr_420px] lg:items-center lg:gap-8">
      <div className="reveal-up flex flex-col">
        ...
        <div className="mt-8 hidden lg:flex flex-col gap-3 sm:flex-row">
          <Link href="/builder" className="action-button">5분 만에 시작하기 <span aria-hidden="true">→</span></Link>
          <button onClick={() => setOpen(true)} className="quiet-button">스마트폰에 설치하는 방법</button>
        </div>
      </div>
      ...
      <div className="reveal-up flex flex-col gap-3 sm:flex-row lg:hidden">
        <Link href="/builder" className="action-button">5분 만에 시작하기 <span aria-hidden="true">→</span></Link>
        <button onClick={() => setOpen(true)} className="quiet-button">스마트폰에 설치하는 방법</button>
      </div>
