...
본 문서는 사용자가 특정 프로젝트나 서비스(예: *"미용실 예약 매장"*, *"피트니스 클럽"*, *"온라인 클래스"*, *"스터디카페"* 등)를 지정했을 때, 해당 서비스 구현에 필요한 블록 구성을 도출하고, 100% 원자 컴포넌트 조립 방식으로 필요한 블록 자산을 새로 생성하거나 기존 블록/원자를 고도화하기 위한 AI 서브에이전트 전용 지침서입니다.
...
* **핵심 목적**:
  - 특정 산업군 및 다양한 서비스 유즈케이스를 결함 없이 100% 구현해 낼 수 있도록 **블록 생태계 자산을 지속해서 확충하고 고도화(Build & Refine Block Assets)**한다.
* **수행 프로토콜 요약**:
  1. 지정된 서비스/프로젝트 구현에 필요한 전체 블록 청사진(List Up) 도출.
  2. 기존 19종+ 블록과의 매핑 분석 (재사용 가능 여부 판별).
...
### 1단계: 유저 프로젝트 분석 및 블록 청사진(List-Up) 도출
* 사용자가 요청한 대상 서비스(예: *"피트니스 클럽 웹사이트"*)를 파악.
* 해당 웹사이트를 구성하는 핵심 섹션 리스트업:
  * 예: 히어로 섹션 + 스케줄/타임슬롯 + 가격표 + 시설 갤러리 + 지도/위치 + FAQ + 카운트다운 + 문의 폼

### 2단계: 기존 블록 매핑 및 수정/신규 판별
* **경로 A (기존 블록 수정/고도화)**: 기존 블록(`blk_consulting_slot_01`, `blk_pricing_01` 등)이나 기존 원자 컴포넌트의 Props/Variant만 보완하여 해결 가능한 경우, 해당 블록/원자를 고도화.
* **경로 B (신규 블록 생성)**: 기존 자산으로 대체 불가능한 특수 표현 섹션인 경우, `src/components/atoms/` 조합으로 `blk_[기능명]_01.tsx` 새로 구현.

### 3단계: 빌더 시스템 2곳 연동
1. `src/components/builder/BuilderCanvas.tsx` 내 `BlockRegistry` 등록.
2. `src/app/builder/page.tsx` 내 `ALL_BLOCKS` 팔레트에 등록.
