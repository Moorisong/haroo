...
### 블록 티어별 제작비 단가표 ( 일시불 )
* **STARTER 티어 (99,000원)**: Starter 15종 블록, 무제한 무료 수정, haroo.site 기본 서브도메인, **최대 3개 화면(페이지)**.
* **STANDARD 티어 (199,000원)**: Starter + Standard 25종 블록 (예약/문의 폼, 카톡/LMS), 커스텀 도메인 무상 매핑, **최대 10개 화면(페이지)**.
* **PROFESSIONAL 티어 (299,000원)**: 전체 45종 블록 (소셜로그인, PG결제, 통계), 커스텀 도메인 & 무제한 SSL 자동 발급, **최대 20개 화면(페이지)**.
